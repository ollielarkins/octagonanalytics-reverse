...
function getPrompt(name: string, args: any): any | null {
  ...
  if (name === "billing") {
    return { description: "Quarterly billing scorecard", ...msg("Call the billing tool. Present each recruiter's quarterly target vs their Won-this-quarter and their open pipeline value, as a compact table with a 'pipeline vs target' coverage read. LEAD with the caveat: Won-fee data is currently very sparse, so won_qtr reads near zero and the open pipeline value is the meaningful leading indicator — do not present anyone as 'failing' on Won alone. Flag who has thin pipeline relative to their target (they need to build pipeline) and who has strong coverage. Recruiters without a loaded target (e.g. Will) — note target TBC.") };
  }
  if (name === "day_plan") {
